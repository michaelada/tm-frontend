...
export function validateQuantity(quantity: number, minQuantity: number, groupOfQuantity: number ) {
  if (!quantity || Number.isNaN(quantity)) {
      return "Quantity cannot be empty";
  }
  if (quantity < minQuantity) {
      return `Quantity cannot be less than ${minQuantity}`;
  }

  if (groupOfQuantity > 1 && quantity % groupOfQuantity !== 0) {
      return `Quantity must be a multiple of ${groupOfQuantity}`;
  }
  return "";
}
